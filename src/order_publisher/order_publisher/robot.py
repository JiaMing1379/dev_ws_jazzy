...
class Robot(Node):
    ...
    def order_callback(self, msg: Order):
        ...
        for step in msg.steps:
            ...
            self.get_logger().info(f"Task Type: {step.task_type}")
            self.get_logger().info(f"Location Name: {step.location_name}")
            self.get_logger().info(f"Step Target Pose: {step.target_pose.position.x}, {step.target_pose.position.y}, {step.target_pose.position.z}")

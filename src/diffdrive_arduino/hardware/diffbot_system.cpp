  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  cfg_.motor_type = std::stoi(info_.hardware_parameters["motor_type"]);
  cfg_.motor_deadband = std::stoi(info_.hardware_parameters["motor_deadband"]);
  cfg_.motor_phase_lines = std::stoi(info_.hardware_parameters["motor_phase_lines"]);
  cfg_.motor_reduction_ratio = std::stoi(info_.hardware_parameters["motor_reduction_ratio"]);
  cfg_.wheel_diameter = std::stof(info_.hardware_parameters["wheel_diameter"]);
  ...
  wheel_l_.setup(cfg_.left_wheel_name, cfg_.enc_counts_per_rev, cfg_.wheel_diameter);
  wheel_r_.setup(cfg_.right_wheel_name, cfg_.enc_counts_per_rev, cfg_.wheel_diameter);
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  comms_.set_motor_type(cfg_.motor_type);
  comms_.set_motor_deadband(cfg_.motor_deadband);
  comms_.set_motor_phase_lines(cfg_.motor_phase_lines);
  comms_.set_motor_reduction_ratio(cfg_.motor_reduction_ratio);
  comms_.set_wheel_diameter(cfg_.wheel_diameter);
...
hardware_interface::CallbackReturn DiffDriveArduinoHardware::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  comms_.stop_motors(); // Stop motors
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  int motor_l_vel = wheel_l_.cmd*wheel_l_.diameter/2; //* wheel_l_.rads_per_count / cfg_.loop_rate;
  int motor_r_vel = wheel_r_.cmd*wheel_r_.diameter/2; // * wheel_r_.rads_per_count / cfg_.loop_rate;
  comms_.set_motor_values(motor_l_vel, motor_r_vel);
  return hardware_interface::return_type::OK;
}
